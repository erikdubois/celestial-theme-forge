...
class PickerWindow(Gtk.ApplicationWindow):
    ...
    def _build_worker(self, name, hexv):
        ...
        # honours THEME_VARIANTS, install takes -t <name>. -k adds the Qt/Kvantum
        # theme; --kde installs the KDE Plasma theme (colour scheme, global theme,
        # desktop theme, Aurorae) so the colour is selectable in System Settings.
        steps = [
            ...
            ("install", [os.path.join(CELESTIAL_DIR, "install.sh"), "-t", name, "-k", "--kde"],
             CELESTIAL_DIR, {}),
        ]
        ok = True
